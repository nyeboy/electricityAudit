...
app.controller('ownerManagerCtrl', ['lsServ',  '$rootScope', '$scope', '$state','$stateParams', 'ngDialog', 'utils', 'commonServ', function (lsServ, $rootScope, $scope, $state,$stateParams, ngDialog, utils, commonServ) {
	...
	var URL=commonServ.exportExcel();
	...
	input.attr("value","业主信息");
	$("body").append(form);
	form.append(input);
	form.submit();
	 
    	
//		   $scope.ownerExportDialog=ngDialog.open({
//	            template: './tpl/ownerExportDialog.html?time='+new Date().getTime(),
...
//				});
    ...
    //导出excle(业主信息)--弃用
	 $scope.ownerExcel=function(){
		
		event.preventDefault();
       var BB = self.Blob;
       var contentStr = document.getElementById("ownerDetails").innerHTML;   //内容
       var fileNmae='业主信息.xls';
       saveAs(
         new BB(
